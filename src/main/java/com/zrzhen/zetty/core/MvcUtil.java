...
import com.zrzhen.zetty.core.mvc.exception.RequestBodyShouldNotBeEmptyException;
import com.zrzhen.zetty.core.mvc.exception.RequestParamShouldNotBeEmptyException;
import com.zrzhen.zetty.core.mvc.exception.UriNotFoundException;
...
public class MvcUtil {
    ...
    public static void handle(AsynchronousSocketChannel ch, Request request) {
        ...
        try {
            ...
            /*设置返回头中的内容大小*/
            if (out.getBody() != null) {
                out.getHeaders().put(HttpHeaders.Names.CONTENT_LENGTH, String.valueOf(out.getBody().length));
            } else {
                log.error("Response body is null!");
            }

            ByteBuffer byteBuffer = out.toByteBuffer();
            write(ch, byteBuffer, false);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            writeError(ch, HttpResponseStatus.INTERNAL_SERVER_ERROR);
        } finally {
            Request.remove();
            Response.remove();
        }
        ...
        if (controllerMethod == null) {
            ...
            if (response.isFlag()) {
                ...
                if (contentTypeEnum.equals(ContentTypeEnum.HTML)) {
                    ...
                    /*默认返回字符串*/
                    response.setBody(FileUtil.str2Byte(String.valueOf(result)));
                    setContentType(response, contentTypeEnum);
                    setConnetion(response, request);
                }
            }
     ...
     * @param writeBuffer
     */
    public static void write(AsynchronousSocketChannel channel, ByteBuffer writeBuffer, boolean isKeepAlive) {
        writeBuffer.flip();
        //异步写数据 参数与前面的read一样
        channel.write(writeBuffer, writeBuffer, new WriteHandler(channel, isKeepAlive));
    }
     ...
     * @param channel
     */
    public static void writeError(AsynchronousSocketChannel channel, HttpResponseStatus status) {
        Request request = Request.get();
        Response response = new Response();
        if (request == null) {
        ...
        write(channel,byteBuffer,false);
    }
