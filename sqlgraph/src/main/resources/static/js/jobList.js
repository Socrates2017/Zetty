var app = new Vue({
    ...
    created: function () {
        ...
        this.project.id = window.location.pathname.split('/').pop();

        this.getAllJobs(this.project.id, 0);
    ...
    mounted: function () {
        //this.getAllJobs(window.location.pathname.split('/').pop(), 0);
    },
    methods: {
        ...
        getAllJobs: function (project_id, page) {
            $("#all").addClass("active");
            ...
            this.postUtil({
                path: '/data/jobsByProject/' + project_id,
                data: {
                    page: page
                },
        ...
        getProject: function (projectId) {
            this.postUtil({
                path: '/data/getProject/' + projectId,
                success: function (data) {
                    app.project.name = data.name;
                    app.project.description = data.description;
                },
                error: function (error) {
        ...
        getJobsByStatus: function (project_id, page, status) {
            ...
            this.postUtil({
                path: '/data/getJobsByStatus/' + status + "?project_id=" + project_id,
                data: {
                    page: page
                },
                success: function (data) {
        ...
        resumeDeleteJob: function (index) {
            this.openConfirm({
                title: '确认恢复任务',
                content: "此操作会恢复任务到未上线状态",
                yesFunc: function () {
                    app.jobLoaderShow(index);
                    app.postUtil({
                        path: '/scheduler/resumeDeleteJob',
                        data: {
                            jobId: app.jobs[index].id
                        },
                        success: function (data) {
