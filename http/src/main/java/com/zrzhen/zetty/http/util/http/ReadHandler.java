...
import com.zrzhen.zetty.http.MvcUtil;
import com.zrzhen.zetty.http.http.*;
import com.zrzhen.zetty.http.util.ExecutorUtil;
...
public class ReadHandler implements CompletionHandler<Integer, ByteBuffer> {
    ...
    public void execute(ByteBuffer attachment) {
        attachment.flip();
        try {
            if (socketStatus.equals(HttpSocketStatus.SKIP_CONTROL_CHARS)) {
                skipControlCharacters(attachment);
                this.socketStatus = HttpSocketStatus.READ_INITIAL;
            }
            if (socketStatus.equals(HttpSocketStatus.READ_INITIAL)) {
                ...
                this.socketStatus = HttpSocketStatus.READ_HEADER;
            }
            if (socketStatus.equals(HttpSocketStatus.READ_HEADER)) {
                ...
                if (contentLength > 0) {
                    ...
                    this.socketStatus = HttpSocketStatus.READ_VARIABLE_LENGTH_CONTENT;
                } else {
                    this.socketStatus = HttpSocketStatus.READ_REQUEST_FINISH;
                }
            }
            if (socketStatus.equals(HttpSocketStatus.READ_VARIABLE_LENGTH_CONTENT)) {
                try {
            ...
            if (socketStatus.equals(HttpSocketStatus.READ_REQUEST_FINISH)) {
